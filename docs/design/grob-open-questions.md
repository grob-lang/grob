...
> the deferred work register — decided work with a named owner and a
...
| R-10 | **`docs/errors/Exxxx.md` written.** 121 codes, zero documents; `--explain Exxxx` is specified to read them. Four sections each: cause, example, fix, see-also. | v1 requirements §10 | `unowned` — v1 release gate | A document exists for every code in the registry, and `--explain` resolves for all 121 |
