...
3. `TypeChecker.TypeCycles.cs`'s §17.1/D-287 required-field cycle-detection DFS
   ...
   `statusCode: int?`), so they can never be a cycle source or target.
...
2. The global-scope dictionary is pre-sized to `3 + ExceptionHierarchy.AllNames.Count`
   (builtins plus hierarchy); `UserTypeRegistry`'s dictionary is pre-sized to
   `ExceptionHierarchy.AllNames.Count`. Both still grow normally for further user-declared
   names; only the resize-and-copy cost for the known fixed load is removed.
3. `DetectTypeCycles` filters `_userTypeRegistry.AllTypes` to exclude
