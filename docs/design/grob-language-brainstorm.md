...
> **Note (April 2026, D-288):** The final shape of `const` is split into two
> keywords — `const` for compile-time constants (literals, arithmetic on
> literals) and `readonly` for runtime-once bindings. See `grob-language-
> fundamentals.md` §24. The `const MAX := 100` example above is still correct
> under the new rule (the RHS is a literal).
...
> `grob-decisions-log.md` for current status.
...
record is `grob-decisions-log.md`. This document is preserved for
historical context.*
