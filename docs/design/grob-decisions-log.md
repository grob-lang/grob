...
call path — the second of its two designed consumers (the namespace-native path,
`date.parse`, was the first) — and lands the three `string` methods D-363 deferred for
...
namespace-native proving case, the first of its two consumers), D-358 (the ratified
...
_NativeDefaultArgumentFill into the primitive-member call path — the second of its two_
_designed consumers — landing padLeft/padRight/truncate, the three string methods_
